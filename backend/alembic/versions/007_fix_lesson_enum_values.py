...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE lessons ALTER COLUMN status TYPE lessonstatus USING LOWER(status)::lessonstatus"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE lessons ALTER COLUMN status TYPE lessonstatus USING UPPER(status)::lessonstatus"
    )

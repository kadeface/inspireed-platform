...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('draft', 'submitted', 'graded', 'returned', name='activitysubmissionstatus'), nullable=False, server_default='draft'),
    ...
    op.create_index('idx_activity_sub_cell_student', 'activity_submissions', ['cell_id', 'student_id'])
    op.create_index('idx_activity_sub_lesson_status', 'activity_submissions', ['lesson_id', 'status'])
    ...
    op.create_table(
        ...
        sa.Column('lesson_id', sa.Integer(), nullable=False),
        sa.Column('cell_id', sa.Integer(), nullable=False),
        sa.Column('review_data', postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default='{}'),
        sa.Column('score', sa.Float(), nullable=True),
        sa.Column('max_score', sa.Float(), nullable=True),
        sa.Column('comment', sa.Text(), nullable=True),
        sa.Column('status', sa.Enum('pending', 'in_progress', 'completed', name='peerreviewstatus'), nullable=False, server_default='pending'),
        ...
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['submission_id'], ['activity_submissions.id'], ),

...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('difficulty_level', sa.Enum('beginner', 'intermediate', 'advanced', name='difficultylevel'), nullable=False, comment='难度等级'),
    ...
    op.create_index(op.f('ix_learning_path_lessons_id'), 'learning_path_lessons', ['id'], unique=False)

    # 给Lesson表添加新字段
    op.add_column('lessons', sa.Column('difficulty_level', sa.Enum('beginner', 'intermediate', 'advanced', name='difficultylevel'), nullable=True, comment='难度等级'))
    op.add_column('lessons', sa.Column('average_rating', sa.Float(), nullable=False, server_default='0.0', comment='平均评分'))
    op.add_column('lessons', sa.Column('review_count', sa.Integer(), nullable=False, server_default='0', comment='评论数量'))

...
def upgrade() -> None:
    ...
    op.add_column('lessons', sa.Column('cell_count', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('lessons', sa.Column('estimated_duration', sa.Integer(), nullable=True))
    op.add_column('lessons', sa.Column('view_count', sa.Integer(), nullable=False, server_default='0'))
    
    op.create_index(op.f('ix_lessons_reference_resource_id'), 'lessons', ['reference_resource_id'], unique=False)
    op.create_foreign_key('fk_lessons_reference_resource_id', 'lessons', 'resources', ['reference_resource_id'], ['id'])

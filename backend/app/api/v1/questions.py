...
from app.schemas.question import (
    QuestionCreate, QuestionUpdate, QuestionResponse, QuestionListItem, QuestionListResponse,
    AnswerCreate, AnswerUpdate, AnswerResponse, VoteCreate, RatingCreate,
    UserBrief, LessonBrief, CellBrief, QuestionStats
)
...
def build_question_list_item(question: Question, answer_count: int = 0) -> dict:
    ...
    return {
        **question.__dict__,
        ...
        "answer_count": answer_count or len(question.answers) if hasattr(question, 'answers') else 0,
...
async def get_my_questions(
    ...
    # 总数
    count_query = select(func.count()).select_from(Question).where(Question.student_id == current_user.id)
    if lesson_id:
...
async def get_question_detail(
    ...
    if not question.is_public and question.student_id != current_user.id and current_user.role.value != "teacher":
...
async def get_teacher_pending_questions(
    ...
    query = select(Question).where(
        or_(
            Question.ask_type == AskType.TEACHER,
            Question.ask_type == AskType.BOTH
        )
    )

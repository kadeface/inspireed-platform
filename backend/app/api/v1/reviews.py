...
from app.schemas.review import (
    ...
    ReviewWithUser, LessonRatingStats
)
...
def create_review(
    *,
    ...
    existing = db.query(Review).filter(
        ...
        Review.lesson_id == review_in.lesson_id
    ).first()
    
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="已经评论过该课程，请使用更新接口"
        )
...
def _update_lesson_rating(db: Session, lesson_id: int):
    ...
    ).filter(Review.lesson_id == lesson_id).first()

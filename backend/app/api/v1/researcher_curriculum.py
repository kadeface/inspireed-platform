...
from app.schemas.curriculum import (
    SubjectResponse, SubjectToggle,
    GradeResponse, GradeToggle,
    CourseCreate, CourseUpdate, CourseResponse,
    CurriculumTreeResponse, SubjectTreeNode, GradeTreeNode, CourseTreeNode
)
...
async def create_course(
    ...
    grade_result = await db.execute(
        select(Grade).where(Grade.id == course_in.grade_id)
    )
...
async def delete_course(
    ...
    if lesson_count.scalar() > 0:
        raise HTTPException(
            status_code=400, 
            detail="该课程下有教案，无法删除。请先删除或移动相关教案。"
        )
    ...
    for subject in subjects:
        ...
        for grade in grades:
            ...
            if key in course_map:
                ...
                grade_nodes.append(GradeTreeNode(
                    id=grade.id,
                    name=grade.name,
                    level=grade.level,
        ...
        if grade_nodes or include_inactive:
            subject_nodes.append(SubjectTreeNode(
                id=subject.id,
                name=subject.name,
                code=subject.code,

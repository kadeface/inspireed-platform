...
async def query_teacher_assistant(
    ...
    """
    logger.info(
        "AI assistant request: user=%s topic=%s question_len=%d",
        current_user.username, payload.topic, len(payload.question or ""),
    )
    ...
    if payload.context and payload.context.agent_prompt:
        ...
        logger.debug("Using custom agent prompt (len=%d)", len(agent_prompt))
    ...
    logger.info(
        ...
        ai_result.model_used, ai_result.confidence, len(ai_result.answer),
    )

...
class SubjectGroup(Base):
    ...
    # 学科关联
    subject_id = Column(Integer, ForeignKey("subjects.id"), nullable=False, index=True, comment="关联学科")
    
    # 年级关联（可选，用于筛选）
    grade_id = Column(Integer, ForeignKey("grades.id"), nullable=True, index=True, comment="关联年级（可选）")
    ...
    shared_lessons = relationship("SharedLesson", back_populates="group", cascade="all, delete-orphan")
...
class GroupMembership(Base):
    ...
    # 关联
    group_id = Column(Integer, ForeignKey("subject_groups.id"), nullable=False, index=True, comment="教研组ID")
...
class SharedLesson(Base):
    ...
    # 关联
    group_id = Column(Integer, ForeignKey("subject_groups.id"), nullable=False, index=True, comment="教研组ID")
    lesson_id = Column(Integer, ForeignKey("lessons.id"), nullable=False, index=True, comment="教案ID")

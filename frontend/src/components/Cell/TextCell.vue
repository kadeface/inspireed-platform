...
<template>
  ...
  >
    ...
    <div v-else class="text-cell-editor">
      <div class="flex justify-between items-center mb-2">
        ...
        <div class="flex gap-2">
          ...
          <div
            ...
            class="flex items-center gap-1 border border-gray-300 rounded"
          >
            <button
      ...
      >
        <MarkdownEditor v-model="tempMarkdown" @update:modelValue="handleMarkdownUpdate" />
      </div>
      <!-- 预览模式 -->
      <div
        v-else
      ...
      >
        ...
        >
          ...
          <span
            >紧凑模式已启用，<span class="underline">{{
              editable ? '点击前往顶部关闭' : '点击前往顶部切换'
            }}</span></span
          >
        </button>
      </div>
...
const sanitizedHtml = computed(() => {
  ...
  if (!props.editable && html) {
    // 定义占位符文本
    const placeholderTexts = ['在此输入文本内容...', 'Enter text content here...']

    // 先检查是否有图片或其他实际内容
    const hasImage = /<img[^>]*>/i.test(html)
    const hasFile = /<a[^>]*class="file-[^"]*"[^>]*>/i.test(html)
    const hasOtherContent = /<(?:h[1-6]|ul|ol|li|blockquote|code|pre|table|div|span)[^>]*>/i.test(
      html
    )
      ...
      if (
        placeholderTexts.some((placeholder) => {
          return (
            ...
            html.trim() === `<p>${placeholder}</p>` ||
            html.trim() === placeholder
          )
        })
      ) {
        return ''
  ...
  if (html) {
    ...
    imageMatches.forEach(
      ({ normalizedSrc, filename, startIndex, endIndex, resourcePath, resourceId }) => {
  ...
  html = html.replace(
    /<a[^>]*class="file-download-btn"[^>]*href\s*=\s*(["'])([^"']+)\1[^>]*>/gi,
    (match, quote, url) => {
      let newUrl = url
      if (url.startsWith('/') && !url.startsWith('//')) {
        newUrl = `${baseURL}${url}`
...
function handleUpdate() {
  ...
  if (cellToSave.content?.html) {
    ...
    html = html.replace(/<img\s+([^>]*?)>/gi, (match, attrs) => {
      const srcMatch =
        attrs.match(/\ssrc\s*=\s*(["'])([^"']+)\1/i) || attrs.match(/\ssrc\s*=\s*([^\s>]+)/i)
      if (srcMatch) {
        const quote = srcMatch[1] || '"'
        let src = srcMatch[2] || srcMatch[1]
...
/* 全屏按钮样式 */
.cell-fullscreen-btn {
  @apply absolute top-2 right-2 p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded opacity-0 transition-all duration-200 z-10;
}

.text-cell:hover .cell-fullscreen-btn,
.text-cell.fullscreen .cell-fullscreen-btn {
  @apply opacity-100;
}
...
.text-cell.fullscreen {
  @apply fixed inset-0 z-50 bg-white overflow-auto rounded-none shadow-none m-0 h-full w-full;
}

.text-cell.fullscreen .cell-fullscreen-btn {
  @apply fixed top-4 right-4 bg-white/80 shadow-sm backdrop-blur-sm border border-gray-200 p-2;
}

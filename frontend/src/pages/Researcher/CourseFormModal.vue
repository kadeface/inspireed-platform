...
watch(
  ...
  async (courseInfo) => {
    if (courseInfo?.course) {
      ...
      const gradeId = course.grade_id ?? courseInfo.grade?.id ?? null
      originalGradeId.value = gradeId ? Number(gradeId) : null
      ...
      showAdvanced.value = !!(course.code || course.description || course.display_order)

...
class CourseExportImportTester:
    ...
    def test_download_template(self) -> bool:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v1/course-export/export-template"
            )
    ...
    def test_export_all_courses(self) -> bool:
        """测试导出所有课程"""
        try:
            params = {"include_lessons": "true", "include_resources": "true"}

            response = self.session.get(
                f"{self.base_url}/api/v1/course-export/export-all", params=params
            )
    ...
    def test_export_single_course(self, course_id: int) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ 课程 {course_id} 导出失败: {response.status_code} - {response.text}"
                )
                return False
    ...
    def run_full_test(self):
        ...
        # 清理测试文件
        test_files = ["course_export_template.json", "all_courses_export.json"]
